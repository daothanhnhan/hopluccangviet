...
class action_page extends library{
	...
	/*-------------- Cấu hình trang chủ --------------------*/
		...
		public function updateConfig(){
			...
			$link_slide1 = isset($_POST['link_slide1']) ? $_POST['link_slide1'] : '';

			$link_slide2 = isset($_POST['link_slide2']) ? $_POST['link_slide2'] : '';

			$link_slide3 = isset($_POST['link_slide3']) ? $_POST['link_slide3'] : '';
...
// echo json_encode(array('error'=>'Lỗi, mời thử lại'));die;
			...
			if (!$this->Update_1('config',
								  ...
								  array('web_name', 
    							 	    ...
    							 	    'google', 'link_slide1', 'link_slide2', 'link_slide3', 'link_banner',
	...
	/*--------------- Danh mục sản phẩm -------------------*/
		...
		public function updateProductCat(){
	    	...
	    	if (isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != "") {
		    	...
		    	$img = $_FILES['fileUpload2'];

		    	$this->uploadPicture($src, $img['name'], $img['tmp_name']);	

		    	$img_name_sub = $img['name'];

	    	}else{

	    		$img_name_sub = $_POST['productcat_sub'];
	    	...
	    	if (isset($_FILES['fileUpload3']) && $_FILES['fileUpload3']['name'] != "") {
		    	...
		    	$productcat_banner_img = $img['name'];

	    	}else{

	    		$productcat_banner_img = $_POST['productcat_banner_img'];
	    	...
	    	if (isset($_FILES['fileUpload4']) && $_FILES['fileUpload4']['name'] != "") {
		    	...
		    	$productcat_main_img = $img['name'];

	    	}else{

	    		$productcat_main_img = $_POST['productcat_main_img'];
	    	...
	    	if (isset($_FILES['fileUpload5']) && $_FILES['fileUpload5']['name'] != "") {
		    	...
		    	$productcat_sub_img = $img['name'];

	    	}else{

	    		$productcat_sub_img = $_POST['productcat_sub_img'];
	    	...
	    	if(isset($_FILES['fileUpload21']) && $_FILES['fileUpload21']['name'] != ""){

	    		$src= "../images/";

				foreach (array_combine($_FILES['fileUpload21']['name'], $_FILES['fileUpload21']['tmp_name']) as $name => $tmp_name) {
	    	...
	    	if (!$this->Update_1('productcat', 
	    						  ...
	    						  array('productcat_name', 
	    						  	    ...
	    						  	    'productcat_sub', 'productcat_main_img', 'productcat_sub_img', 

	    						  	    'productcat_banner_img',
	    						  ...
	    						  array(&$productcat_name, 
	    						  	    ...
	    						  	    &$img_name_sub, &$productcat_main_img, &$productcat_sub_img, 
		...
		public function deleteProductCat(){
			...
			if ($productcat_id == 423) {
				echo json_encode(array('error'=>'Muc này không được xóa.'));die;
			}
   ...
   //          }
	    	...
	    	if (isset($_FILES['fileUpload3']) && $_FILES['fileUpload3']['name'] != "") {
		    	...
		    	$img_2 = $_FILES['fileUpload3'];

		    	$this->uploadPicture($src, $img_2['name'], $img_2['tmp_name']);	

		    	$img_name_2 = $img_2['name'];

	    		$success = $this->createThumbnail($src.$img_name_2, $src.'/thumbs/'.$img_name_2, 270, 190,array(255,255,255)); // creates a thumbnail 
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			$insert = $this->Insert_1('product', 
									   ...
									   // 'sssssssdddssssisiiissssssssssssssssissssi');
									   'sssssssdddssssisiiissssssssssssssssissssi');
...
// echo json_encode(array('error'=>'Lỗi o day'));die;
			...
			$product_sort = isset($_POST['product_sort']) ? $_POST['product_sort'] : '';
   ...
   //          }
			...
			if (isset($_FILES['fileUpload1']) && $_FILES['fileUpload1']['name'] != "") {
	    		...
	    		$img_name = isset($_POST['product_img']) ? $_POST['product_img'] : '';
	    	...
	    	if (isset($_FILES['fileUpload3']) && $_FILES['fileUpload3']['name'] != "") {
	    		...
	    		$img_name_2 = isset($_POST['product_img_2']) ? $_POST['product_img_2'] : '';
	    	...
	    	if (isset($_FILES['fileUpload4']) && $_FILES['fileUpload4']['name'] != "") {
		    	...
		    	$img_video = $_FILES['fileUpload4'];

		    	$this->uploadPicture($src, $img_video['name'], $img_video['tmp_name']);	

		    	$img_name_video = $img_video['name'];

	    		// $success = $this->createThumbnail($src.$img_name_2, $src.'/thumbs/'.$img_name_2, 270, 190,array(255,255,255)); // creates a thumbnail 

	    	}else{

	    		$img_name_video = isset($_POST['product_img_video']) ? $_POST['product_img_video'] : '';
			...
			if (!$this->Update_1('product',
								  ...
								  'sssssssdddssssisiiissssssssssssssssisissssi')) {
